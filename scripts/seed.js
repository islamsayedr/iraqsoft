  ...
  try {
    ...
    const insertedUpdates = await Promise.all(
      updates.map(async (update) => {
        const id = uuidv4();
        return client.sql`
        INSERT INTO updates (id, date, title, des, cover)
        VALUES (${id}, ${update.date}, ${update.title}, ${update.des}, ${update.cover})
        ON CONFLICT (des) DO NOTHING;
      `;
      })
    );

    console.log(`Seeded ${insertedUpdates.length} update`);
